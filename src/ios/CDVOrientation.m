    ...
    if([vc respondsToSelector:selector]) {
        ...
        if ([UIDevice currentDevice] != nil){
            ...
            if (value != nil) {
                ...
                //解决bug：从播放视频页面返回课程简介页面时，横屏状态无法恢复成竖屏。
                UIDeviceOrientation previousOrientation = [UIDevice currentDevice].orientation;
                if (value.intValue == previousOrientation) {
                    [[UIDevice currentDevice] setValue:[NSNumber numberWithInt:previousOrientation + 2 > 4 ? previousOrientation - 2 : previousOrientation + 2 ] forKey:@"orientation"];
                }
                //解决bug：从播放视频页面返回课程简介页面时，横屏状态无法恢复成竖屏。  end
